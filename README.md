...
1. **Clone or Download:** Get the server code. If it's in a repository:

    ```bash
    git clone <repository-url>
    cd near-mcp-server-full
    ```

    If you only have the `index.ts` file, create the project structure as described in the previous step.
...
3. **Configure Environment Variables:**
    Create a `.env` file in the root of the project directory. **Important:** Add `.env` to your `.gitignore` file to avoid accidentally committing your secret seed phrase!
...
4. **Build the Server:** Compile the TypeScript code to JavaScript.
...
2. **Edit Config:** Add an entry for this server under `mcpServers`. **Use the absolute path** to the compiled `build/index.js` file in your project directory.
    ...
    _Replace `/path/to/your/project/near-mcp-server-full` with the actual path on your system._
  ...
  ```json
  {
    "mcpServers": {
      "near-protocol-server": {
        "command": "node",
        "args": [
          "/path/to/your/project/near-mcp-server-full/build/index.js"
        ],
        // Optional: Pass environment variables directly if needed,
        // though reading from .env is preferred for the server itself.
        // "env": {
        //   "MNEMONIC": "...",
        //   "NEAR_NETWORK_ID": "testnet",
        //   "NEAR_ACCOUNT_ID": "your-account.testnet"
  ...
  ```json
  {
    "mcpServers": {
      "near-protocol-server": {
        "command": "node",
        "args": [
          "C:\\path\\to\\your\\project\\near-mcp-server-full\\build\\index.js"
        ]
        // Optional: Pass environment variables directly if needed
        // "env": { ... }
      }
...
4. **Verify Connection:** Look for the hammer icon (<img src="https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/claude-desktop-mcp-hammer-icon.svg" style="display: inline; margin: 0; height: 1.3em;" />) in the chat input area. Clicking it should list the NEAR tools defined in this server.
...
- **Not Appearing in Client (Claude):**
  ...
  - Check Claude's MCP logs (see [Debugging Guide](https://modelcontextprotocol.io/docs/tools/debugging#getting-logs-from-claude-for-desktop)).
